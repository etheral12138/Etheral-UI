<template>
  <div class="et-upload-dragger" :class="{ ['et-upload-draggerenter']: isEnter }" ref="fileArea"
    @click="emits('fileUpload')">
    <div class="et-upload-content">
      <Icon class="et-upload-icon" :name="isEnter ? 'file-open' : 'folder-close'" />
      <div class="et-upload-dragger-text">将文件拖到此处或<em>点击上传</em></div>
    </div>
  </div>
</template>
