<template>
    <div class="et-textarea" v-if="attrs.type === 'textarea'">
        <textarea class="et-textarea__inner" :style="textareaStyle" v-bind="attrs" ref="textarea"
    ...
    <div v-else class="k-input" @mouseenter="isEnter = true" @mouseleave="isEnter = false" :class="styleClass">
        <div class="et-input__prepend" v-if="slots.prepend">
            <slot name="prepend"></slot>
        </div>

        <input ref="ipt" class="et-input__inner" :class="inputStyle" :disabled="inputProps.disabled" v-bind="attrs"
            :value="inputProps.modelValue" @input="changeInputVal" />

        <div class="et-input__append" v-if="slots.append">
            <slot name="append"></slot>
        </div>
        <div @click="clearValue" v-if="inputProps.clearable && isClearAbled" v-show="isEnter" class="et-input__suffix">
            <Icon name="error" />
        </div>
        <div class="et-input__suffix" v-show="isShowEye">
            <Icon @click="changeType" :name="eyeIcon" />
        </div>
        <div class="et-input__prefix" v-if="isShowPrefixIcon">
            <Icon :name="inputProps.prefixIcon" />
        </div>
        <div class="et-input__suffix no-cursor" v-if="isShowSuffixIcon">
...
import { isObject } from '@etheral-ui/utils'
...
const styleClass = computed(() =>
{
    return {
        ...
        [`et-input--${inputProps.size}`]: inputProps.size,
        ['et-input-group k-input-prepend']: slots.prepend,
        ['et-input-group k-input-append']: slots.append
    }
})
const inputStyle = computed(() =>
{
    return {
        ['et-input--prefix']: isShowPrefixIcon.value,
    }
})
